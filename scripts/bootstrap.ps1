...
try {
    ...
    Write-BootstrapLog "Foundation modules loaded: $(@(Get-Module | Where-Object { $_.Name -like 'Ado*' }).Count)."
}
catch {
...
$localFolders = @('.local', 'artifacts/plans', 'artifacts/reports')
...
if ($CheckOnly) {
    ...
    elseif (Test-Path -LiteralPath $envTemplatePath) {
        ...
        Write-BootstrapLog 'created .env from .env.example. Fill in ADO_ORG_URL, ADO_PROJECT and ADO_PAT.'
    }
    else {
