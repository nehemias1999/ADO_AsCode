<#
    AdoAsCode.Configuration - loading and validating declared state.
...
function Import-AdoAsCodeEnvironment {
    ...
    .EXAMPLE
        Import-AdoAsCodeEnvironment -Path '.env'

    .EXAMPLE
        Import-AdoAsCodeEnvironment -Path '.env,automations/team-provisioning/config/members.env'
...
function Resolve-AdoAsCodePath {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        Resolve-AdoAsCodePath -Path 'foundation/config/project-context.json' -RootPath $repoRoot
    #>
    [CmdletBinding()]
    [OutputType([string])]
    param(
...
function Test-AdoAsCodeConfiguration {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        Test-AdoAsCodeConfiguration -Json $raw -SchemaPath $schemaPath
...
function Get-AdoAsCodeConfiguration {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        $projectContext = Get-AdoAsCodeConfiguration -Path 'foundation/config/project-context.json'
    ...
    if (-not $SchemaPath) {
        ...
        $SchemaPath = Resolve-AdoAsCodePath -Path $reference -RootPath (Split-Path -Parent (Resolve-Path -LiteralPath $Path).Path)
    }

    $result = Test-AdoAsCodeConfiguration -Json $raw -SchemaPath $SchemaPath
...
function Get-AdoAsCodeMemberList {
    <#
    .SYNOPSIS
        Reads a membership list from an environment variable.

    .DESCRIPTION
        Membership is the one part of the declared state that must never be
        committed: a list of people's sign-in addresses is personal data with no
        business being in a public history. The configuration therefore names an
        environment variable, and this function reads it.

        Any of comma, semicolon or newline separates entries, because the value is
        typed by hand into a local file.

    .PARAMETER VariableName
        Name of the environment variable holding the list.

    .PARAMETER AllowEmpty
        Return an empty list instead of failing when the variable is not set.
        A Team with no members is a valid intermediate state.

    .EXAMPLE
        Get-AdoAsCodeMemberList -VariableName 'APP_ALPHA_MEMBERS'

    .OUTPUTS
        The list of member identifiers.
    #>
    [CmdletBinding()]
    [OutputType([string[]])]
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($raw)) {
        ...
        throw "Environment variable '$VariableName' is not set, so the Team membership cannot be resolved. Declare it in your local members file or as a pipeline secret."
    }
