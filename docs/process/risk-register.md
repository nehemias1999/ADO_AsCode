...
| 6b | A Variable Group secret is **replaced with the value from another environment** | **Critical** — a working PROD credential is destroyed, the API reports success, and nothing can detect it because the live value was never readable | **High** without a control: the group name carries the environment, the secret's name does not, so one `APP_SERVER_PASSWORD` in `.env` served every environment | A secret is resolved from `<NAME>_<ENVIRONMENT>` only. The bare name requires `-AllowUnqualifiedSecretName`, and the qualified name wins when both exist. Unresolvable means blocked. Seven tests, one of which asserts a DEV value is not used for a PROD group. | Low |
